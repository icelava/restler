...
function Request(uri, options) {
  ...
  this.request = proto.request({
    ...
    headers: this.headers,
	  // Check for custom keepAlive setting in options.
	  // The default behaviour is to pass in undefined agent object,
	  // so that http.ClientRequest uses the globalAgent's settings.
	  // This includes keeping-alive for socket pooling.
	  // To avoid that a false should be passed in.

	  // http://nodejs.org/api/http.html#http_class_http_clientrequest
	  agent: (this.options.keepAlive === false) ? false: undefined
  });

	// WSSSI-605: disable Nagle's algorithm.
